/* Color Dash — Everything Edition
   Updated: add "Customize your cube" corner button inside Play menu and improved skins UI.
*/
...
function endRun(){
  ...
  // Ensure the gameOver container allows pointer events (defensive)
  if (gameOver) {
...
function openMenu(kind){
  ...
  // remove previous customize button, will be re-added only for 'play'
  ...
  if (kind === 'play'){
    ...
    SKINS.forEach(s=>{
      ...
      card.onclick = ()=>{
        saveSkin(s);
        // feedback: highlight briefly then close overlay
...
function addCustomizeCorner(){
  ...
  btn.onclick = (e)=>{
    e.stopPropagation();
    // open skins view inside same overlay
    openMenu('skins');
  };
